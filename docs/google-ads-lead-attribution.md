...
**The saved CRM field is confirmed as `cCGoogleAdsClickId`; the website mapping is corrected locally and awaits deployment.** After the reported rename, the user's successful metadata and third-lead API read still returned `definedFields: ["cCGoogleAdsClickId"]` and a lead attribute with that same name, `matchesExpectedName: false`, `hasValue: false`, and `matchesTest: false`. The deployed website sends `cGoogleAdsClickId`, so the schema mismatch remains in Production. The local route now uses a single `ESPO_GCLID_FIELD = "cCGoogleAdsClickId"` constant for the outgoing payload and response diagnostics. The tests target that confirmed contract and reject the previously assumed key. The CRM field can stay as it is. Persistence with the corrected website mapping is not yet verified; earlier submissions are not replayed or backfilled.
...
The browser's storage and a real request payload are confirmed to contain `Test_Click_123`. The deployed server sends it as `cGoogleAdsClickId`. The earlier field-editor screenshot was incorrectly read as matching that name; the subsequent API reads exposed `cCGoogleAdsClickId`. The field editor showed max length 512. The user reported role Edit: No and later showed newly created leads with a blank click field, so the earlier claim that this permission setting explained the entire failure was premature. Effective API permissions for the confirmed field remain unverified.
...
Next, deploy the corrected website mapping, verify the deployment source, and check persistence through staging or the next genuine enquiry. After deployment, `crm_request.field` must be `cCGoogleAdsClickId`; inspect `fieldReturned`/`fieldMatches` and the corresponding lead. If persistence still fails with the confirmed key, inspect the website API user's effective field Read/Edit permissions and save configuration. No new CRM request, enquiry, push or deployment was performed for this local mapping fix. All three separate one-test approvals have been used; no additional live test is authorized.
